...
// From https://github.com/buildkite/agent/blob/041c26787ef2d1384b489bf2045ff72e5069b923/env/interpolator.go
...
func Interpolate(env []string, str string) (string, error) {
	var err error
	...
	interpolated := variablesWithBracketsRegex.ReplaceAllStringFunc(str, func(v string) string {
		...
		vv, isEnvironmentVariableSet := getEnv(key, env)
...
// Gets an environment key out of the slice
func getEnv(key string, env []string) (string, bool) {
	for _, l := range env {
		parts := strings.SplitN(l, "=", 2)
		if len(parts) == 2 && normalizeKeyName(key) == normalizeKeyName(parts[0]) {
			return parts[1], true
		}
	}
	return "", false
